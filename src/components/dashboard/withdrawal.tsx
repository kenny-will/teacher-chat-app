...
export function WithdrawalPage() {
  ...
  const [recipientType, setRecipientType] = useState<"individual" | "business">(
    "individual",
  );
  ...
  const [country, setCountry] =
    useState<(typeof COUNTRIES)[number]>("United States");
  ...
  const allOutbound = (txnsData ?? []).filter(
  ...
  const activeCrypto =
    CRYPTO_NETWORKS.find((n) => n.id === cryptoNet) ?? CRYPTO_NETWORKS[0];
  ...
  async function handleBankSend() {
    ...
    toast.error("You need to activate your card to process your transaction.")
    setSending(true);
    setFeedback(null);
    try {
      const bankRef = [
      ...
      setFeedback({
        type: "success",
        msg: `${activeCurrency.symbol}${bankFaceAmount.toLocaleString("en-US", { minimumFractionDigits: 2 })} ${currency} withdrawal to ${recipientName.trim()} submitted for approval.`,
      });
      setBankAmount("");
      setBankMemo("");
      setAccountHolder("");
      ...
      setFeedback({
        type: "error",
        msg: "Failed to submit withdrawal. Please try again.",
      });
    } finally {
      setSending(false);
    }
  ...
  async function handleCryptoSend() {
    if (!cryptoAmt || !toAddress || parseFloat(cryptoAmt) <= 0 || sending)
      return;
    setSending(true);
    setFeedback(null);
    try {
      ...
      setFeedback({
        type: "success",
        msg: `${cryptoAmt} ${activeCrypto.symbol} withdrawal submitted for approval.`,
      });
      setCryptoAmt("");
      setToAddress("");
      refetch();
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-12 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-5">
            <SectionHeader
              ...
              subtitle="Fill in details and send for admin approval"
            />
              ...
              <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleCurrencyChange(
                        e.target.value as (typeof CURRENCIES)[number]["code"],
                      )
                    }
                    disabled={isDomesticRail}
                ...
                {isDomesticRail && (
                  <div className="mt-1 text-[10.5px] text-gray-400 dark:text-gray-500">
                    ACH / RTP are USD domestic rails only
                  </div>
                )}
              </div>
            ...
            <div className="mt-5 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 p-4 grid grid-cols-3 gap-3 text-[12px]">
              <div>
                ...
                <div className="font-semibold tabular-nums text-[14px]">
                  {bankAmount
                    ? `${activeCurrency.symbol}${bankFaceAmount.toLocaleString("en-US", { minimumFractionDigits: 2 })} ${currency}`
                    : "—"}
                </div>
                {bankAmount && currency !== "USD" && (
                  <div className="text-[10.5px] text-gray-400 dark:text-gray-500">
                    ≈ $
                    {bankUsdAmount.toLocaleString("en-US", {
          ...
          <div className="col-span-12 lg:col-span-8 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-5">
            ...
            <div className="mt-3 space-y-1">
              {allOutbound.length === 0 ? (
                <div className="py-8 text-center text-[12.5px] text-gray-400 dark:text-gray-500">
                  No withdrawals yet.
                </div>
              ) : (
                allOutbound.map((r) => (
                  ...
                  >
                    ...
                    <div className="col-span-3">
                      <Tag
                        tone={
                          r.statusTone as "amber" | "green" | "rose" | "neutral"
                        }
                      >
                        {r.status}
                      </Tag>
                    </div>
                    <div className="col-span-3 text-[12px] text-gray-700 dark:text-gray-300">
                      {r.transactionDate}
                    </div>
                    <div className="col-span-1 text-right font-semibold tabular-nums text-[12.5px] text-rose-600 dark:text-rose-400">
                      {r.amount}
                    </div>
                  </div>
                ))
          ...
          <div className="col-span-12 lg:col-span-4 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-5">
            <SectionHeader
              title="Pending"
              subtitle={`${pendingOut.length} awaiting`}
              right={
                pendingOut.length > 0 ? (
                  <Tag tone="amber">{pendingOut.length}</Tag>
                ) : undefined
              }
            />
                ...
                pendingOut.map((r) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right tabular-nums font-semibold text-[12.5px] ml-2">
                        {r.amount}
                      </div>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-12 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-5">
            <SectionHeader
              ...
              subtitle="Funds leave within one confirmation"
            />
                ...
                <div className="text-right">
                  {balLoading ? (
                    ...
                    <>
                      ...
                      <div className="text-[11px] text-gray-500 dark:text-gray-400">
                        ≈{" "}
                        {(availableUsd / activeCrypto.usdRate).toFixed(
                          activeCrypto.id === "btc" ? 6 : 4,
                        )}{" "}
                        {activeCrypto.symbol}
                      </div>
                    </>
              ...
              <div>
                ...
                {cryptoAmt && (
                  <div className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                    ≈ $
                    {cryptoUsdAmount.toLocaleString("en-US", {
                      minimumFractionDigits: 2,
                    })}{" "}
                    USD
                  </div>
            ...
            <div className="mt-3 flex items-start gap-2.5 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 px-3.5 py-3">
              ...
              <p className="text-[11.5px] text-amber-700 dark:text-amber-300 leading-relaxed">
                Double-check the address before sending. Crypto transactions are
                irreversible and cannot be recalled.
              </p>
            </div>
          ...
          <div className="col-span-12 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-5">
            ...
            <div className="mt-3 space-y-1">
              {allOutbound.length === 0 ? (
                ...
                allOutbound.map((d) => {
                  const railKey = d.category
                    .toLowerCase()
                    .replace("withdrawal · ", "");
                  const net = CRYPTO_NETWORKS.find(
                    (n) =>
                      n.id === railKey || n.symbol.toLowerCase() === railKey,
                  );
                  return (
                    <div
                      key={d.id}
                    ...
                    >
                      ...
                      <div className="col-span-2">
                        <Tag tone="neutral">
                          {d.category.replace("Withdrawal · ", "")}
                        </Tag>
                      </div>
                      <div className="col-span-3 text-[12px] text-gray-600 dark:text-gray-300 truncate">
                        {d.accountRef ?? "—"}
                      </div>
                      <div className="col-span-2 text-right">
                        <div className="text-[13px] font-semibold tabular-nums text-rose-600 dark:text-rose-400">
                          {d.amount}
                        </div>
                        <Tag
                          tone={
