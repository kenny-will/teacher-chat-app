...
export function AdminDepositsPage() {
  ...
  const filtered = deposits
    .filter((t) => {
      ...
      if (statusFilter === "approved")
        return t.status === "Approved" || t.status === "Paid";
  ...
  const TABS = [
    ...
    { key: "pending" as const, label: `Pending · ${pending.length}` },
    {
      ...
      label: `Approved · ${deposits.filter((t) => t.status === "Approved" || t.status === "Paid").length}`,
    },
    {
      key: "rejected" as const,
      label: `Rejected · ${deposits.filter((t) => t.status === "Rejected").length}`,
    },
  ];
  ...
  return (
    <>
      <PageHeader
        ...
        title="All Deposits."
        subtitle="Inbound transactions across every user — approve or reject pending requests."
        actions={
          <>
            {pending.length > 0 && (
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5">
        ...
        {isLoading ? (
          ...
          <div className="py-16 text-center text-[13px] text-gray-400 dark:text-gray-500">
            ...
            {deposits.length === 0
              ? "No deposit requests from any user yet."
