...
export function AdminNotificationsPage() {
  ...
  if (!selectedUser) return <NoStudentSelected message="Select a student to view their notification preferences." />
    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 dark:border-white/10 p-12 text-center text-gray-400">
          ...
          <div className="text-[12px] mt-1">Notification prefs appear after demo data is loaded.</div>
        </div>
      ) : (
        <div className="grid grid-cols-12 gap-5">
          <div className="col-span-12 lg:col-span-7">
            <Section title={`Notification preferences · ${prefs.length}`}>
              {prefs.length === 0 ? (
                <div className="text-[12px] text-gray-400 py-4 text-center">No preferences configured.</div>
              ) : (
                prefs.map((p) => (
                  <div key={p.id} className="flex items-center justify-between py-3 border-b border-gray-100 dark:border-white/8 last:border-0">
                    <div>
                      <div className="text-[12.5px] font-medium">{p.label}</div>
                      <div className="text-[11px] text-gray-400 mt-0.5">{p.key}</div>
                    </div>
          ...
          <div className="col-span-12 lg:col-span-5">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between text-[12.5px]">
                  ...
                  <span className="font-semibold">
                    {prefs.length > 0 ? Math.round((enabled / prefs.length) * 100) : 0}%
                  </span>
                </div>
              </div>
            </Section>
            ...
            <Section title="About notification prefs">
              <p className="text-[12px] text-gray-500 dark:text-gray-400 leading-relaxed">
                These are the student&apos;s in-app and email notification settings. They control which events the student receives alerts for, such as large transactions, card declines, and compliance alerts.
              </p>
            </Section>
          </div>
        </div>
