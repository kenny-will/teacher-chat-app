...
import { MapPinIcon, RefreshCwIcon, ExternalLinkIcon, WifiOffIcon } from "lucide-react"
import { PageHeader, AvatarBadge, Tag } from "@/components/meridian/primitives"
import { cn } from "@/lib/utils"
import { firebaseReady } from "@/lib/firebase"
import { subscribeToLocations, type UserLocation } from "@/lib/firebase-location"
...
function NotConfigured() {
  return (
    <div className="flex flex-col items-center justify-center h-64 gap-4 text-center">
      ...
      <div>
        <div className="text-[14px] font-semibold text-gray-600 dark:text-gray-300">Firebase not configured</div>
        <div className="text-[12px] text-gray-400 mt-1.5 max-w-xs">
          Add Firebase credentials to <code className="bg-gray-100 dark:bg-white/10 px-1.5 py-0.5 rounded">.env</code> to enable live location tracking.
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-100 dark:border-white/8 last:border-0 hover:bg-gray-50 dark:hover:bg-white/5 transition">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[11.5px] text-gray-500 dark:text-gray-400 truncate">{loc.userEmail}</div>
      </div>

      <div className="text-right shrink-0">
        <div className="text-[12.5px] font-medium">
          {loc.latitude ? `${loc.city}${loc.region ? `, ${loc.region}` : ""}` : "Location unknown"}
        </div>
        <div className="text-[11px] text-gray-400">
          {loc.country}{loc.accuracy ? ` · ±${Math.round(loc.accuracy)}m` : ""}
        </div>
      </div>

      <div className="shrink-0 flex flex-col items-end gap-1">
        <Tag tone={loc.isOnline ? "green" : "neutral"}>{loc.isOnline ? "Online" : "Offline"}</Tag>
        <span className="text-[10.5px] text-gray-400">{timeAgo(loc.updatedAt)}</span>
      </div>
    ...
    <>
      ...
      {!firebaseReady ? (
        ...
        <div className="grid grid-cols-12 gap-5">
          {/* Map */}
          <div className="col-span-12 lg:col-span-8 rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-900 overflow-hidden"
            style={{ height: 460 }}>
            {loading ? (
              ...
              <div className="h-full flex flex-col items-center justify-center gap-3 text-center px-8">
                ...
                <div>
                  <div className="text-[14px] font-medium text-gray-400">No location data yet</div>
                  <div className="text-[12px] text-gray-400 mt-1">
                    Students need to allow location access in their browser. Locations appear here as soon as they connect.
                  </div>
                </div>
              </div>
            ) : (
          ...
          <div className="col-span-12 lg:col-span-4 flex flex-col gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 rounded-xl border border-emerald-200 dark:border-emerald-800/50 bg-emerald-50 dark:bg-emerald-950/20 p-3 text-center">
                <div className="text-[22px] font-semibold text-emerald-600 dark:text-emerald-400">{online.length}</div>
                <div className="text-[11px] text-emerald-700 dark:text-emerald-500">Online now</div>
              </div>
              <div className="flex-1 rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 p-3 text-center">
                <div className="text-[22px] font-semibold text-gray-500">{offline.length}</div>
              ...
              <div className="flex-1 rounded-xl border border-indigo-200 dark:border-indigo-800/50 bg-indigo-50 dark:bg-indigo-950/20 p-3 text-center">
                <div className="text-[22px] font-semibold text-indigo-600 dark:text-indigo-400">{locations.length}</div>
                <div className="text-[11px] text-indigo-700 dark:text-indigo-500">Total</div>
              </div>
            </div>

            {/* Scrollable student list */}
            <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-gray-900 overflow-hidden flex flex-col"
              style={{ maxHeight: 390 }}>
              ...
              <div className="overflow-y-auto flex-1">
                {loading ? (
                  <div className="p-4 space-y-3 animate-pulse">
                    {[0,1,2,3].map((i) => <div key={i} className="h-12 bg-gray-100 dark:bg-white/8 rounded" />)}
                  </div>
                ) : locations.length === 0 ? (
