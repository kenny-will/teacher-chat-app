...
function WithdrawalRow({ txn, onDone, onFlash, highlight }: {
...
function StatusStats({ withdrawals }: { withdrawals: AllTxn[] }) {
  const approved = withdrawals.filter(t => t.status === "Approved" || t.status === "Sent").length
  ...
  return (
    <div className="grid grid-cols-4 gap-3">
      {[
        { label: "Approved / Sent", count: approved, cls: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800" },
...
export function AdminWithdrawalsPage() {
  ...
  const filtered = withdrawals.filter(t => {
    ...
    if (statusFilter === "approved") return t.status === "Approved" || t.status === "Sent"
  ...
  const TABS = [
    ...
    { key: "approved" as const, label: `Approved · ${withdrawals.filter(t => t.status === "Approved" || t.status === "Sent").length}` },
    { key: "rejected" as const, label: `Rejected · ${withdrawals.filter(t => t.status === "Rejected").length}` },
  ]
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Outbound requests across every student — approve, hold, or reject pending withdrawals."
        actions={
          <>
      ...
      {!isLoading && withdrawals.length > 0 && <StatusStats withdrawals={withdrawals} />}
        ...
        {isLoading ? (
          ...
          <div className="py-16 text-center text-[13px] text-gray-400 dark:text-gray-500">
            ...
            {withdrawals.length === 0
              ? "No withdrawal requests from any student yet."
              : "No withdrawals match this filter."}
          </div>
        ) : (
          filtered.map(t => (
            <WithdrawalRow key={t.id} txn={t} onDone={refetch} onFlash={flash} highlight={t.statusTone === "amber"} />
          ))
        )}

        <div className="px-5 py-3 text-[12px] text-gray-500 dark:text-gray-400 border-t border-gray-100 dark:border-white/8">
          {isLoading ? "Loading…" : `Showing ${filtered.length} of ${withdrawals.length} withdrawals`}
        </div>
      </div>
    </>
  )
