...
function WithdrawalStats({ withdrawals }: { withdrawals: Txn[] }) {
...
export function AdminWithdrawalsPage() {
  ...
  if (!selectedUser) return <NoStudentSelected message="Select a student to manage their withdrawals." />

  const withdrawals = (data?.transactions ?? []).filter((t) => t.direction === "outbound")
  const pending     = withdrawals.filter((t) => t.status === "Pending" || t.status === "On Hold")

  return (
    <>
      <PageHeader
        eyebrow="Student · Withdrawals"
        title="Withdrawals."
        subtitle={`Outbound transactions for ${selectedUser.name}`}
        actions={
          <div className="flex items-center gap-2">
            {pending.length > 0 && <Tag tone="amber">{pending.length} pending</Tag>}
            <Tag tone="brand">{withdrawals.length} total</Tag>
          </div>
        }
      />
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 dark:border-white/10 p-12 text-center text-gray-400">
          <ArrowUpRightIcon className="h-8 w-8 mx-auto mb-3 text-gray-200 dark:text-white/15" />
          <div className="text-[14px] font-medium">No data for this student</div>
          <div className="text-[12px] mt-1">Use the Simulate tab to inject a withdrawal.</div>
        </div>
      ) : (
        <>
          ...
          {pending.length > 0 && (
            <div className="rounded-2xl border border-amber-200 dark:border-amber-800/50 bg-white dark:bg-white/5 overflow-hidden mb-5">
              <PendingBanner count={pending.length} label="withdrawal(s)" />
              <TxnTableHeader />
              {pending.map((t) => (
          ...
          {withdrawals.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-gray-200 dark:border-white/10 p-10 text-center text-[12.5px] text-gray-400">
              No withdrawals recorded yet. Use Simulate to inject one.
            </div>
          ) : (
