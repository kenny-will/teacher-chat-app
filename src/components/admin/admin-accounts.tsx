...
const BANK_CURRENCIES   = ["USD", "EUR", "GBP", "SGD", "JPY", "AUD", "CAD", "CHF"]
...
function FeedbackBanner({ fb, onDismiss }: { fb: Feedback; onDismiss: () => void }) {
  ...
  return (
    <div className={cn(
      ...
      <span>{fb.msg}</span>
      <button onClick={onDismiss}><XIcon className="h-3.5 w-3.5" /></button>
    </div>
  )
}
...
function AdjustPanel({ account, onDone, onCancel }: {
  account: Account; onDone: () => void; onCancel: () => void
}) {
  ...
  function handleSubmit() {
    ...
    if (!n || n <= 0) { setFb({ type: "error", msg: "Enter a valid amount." }); return }
    startTransition(async () => {
      try {
        await adminAdjustAccountBalance(account.id, n, direction)
        setFb({ type: "success", msg: `Balance ${direction === "add" ? "increased" : "decreased"}.` })
        setTimeout(onDone, 700)
      } catch (e) {
        setFb({ type: "error", msg: e instanceof Error ? e.message : "Failed." })
      }
    })
  }
  ...
  return (
    <div className="rounded-xl border border-indigo-200 dark:border-indigo-800/60 bg-indigo-50 dark:bg-indigo-950/30 p-3 mt-2 space-y-2.5">
      <div className="text-[11.5px] font-semibold text-indigo-800 dark:text-indigo-300">
        Adjust — {account.name} · <span className="font-mono">{fmtBalance(account)}</span>
      </div>
      {fb && <FeedbackBanner fb={fb} onDismiss={() => setFb(null)} />}
      <div className="flex gap-2">
        <button
          onClick={() => setDirection("add")}
          className={cn("flex-1 h-7 rounded-lg text-[11.5px] font-medium border transition flex items-center justify-center gap-1",
            direction === "add" ? "bg-emerald-600 text-white border-emerald-600" : "bg-white dark:bg-transparent text-gray-700 dark:text-gray-300 border-gray-300 dark:border-white/20")}
        ><PlusIcon className="h-3 w-3" /> Add</button>
        <button
          onClick={() => setDirection("subtract")}
          className={cn("flex-1 h-7 rounded-lg text-[11.5px] font-medium border transition flex items-center justify-center gap-1",
            direction === "subtract" ? "bg-rose-600 text-white border-rose-600" : "bg-white dark:bg-transparent text-gray-700 dark:text-gray-300 border-gray-300 dark:border-white/20")}
        ><MinusIcon className="h-3 w-3" /> Subtract</button>
      </div>
      <input
      ...
      />
      <div className="flex gap-2">
        <Button size="sm" className="flex-1 h-7 text-[11.5px]" onClick={handleSubmit} disabled={pending}>
          {pending ? <RefreshCwIcon className="h-3 w-3 animate-spin" /> : <CheckIcon className="h-3 w-3" />} Confirm
        </Button>
        <Button variant="outline" size="sm" className="h-7 text-[11.5px]" onClick={onCancel} disabled={pending}>Cancel</Button>
      </div>
    </div>
  )
}
...
function AddAccountForm({ userId, type, onDone, onCancel }: {
  userId: string; type: "bank" | "crypto"; onDone: () => void; onCancel: () => void
}) {
  ...
  const [bankName, setBankName] = useState(type === "bank" ? BANK_PRESETS[0].bank : CRYPTO_PRESETS[0].bank)
  const [routing, setRouting]   = useState(type === "bank" ? BANK_PRESETS[0].routing : "")
  const [apy, setApy]           = useState("")
  const [status, setStatus]     = useState<"active" | "earning" | "pending">("active")
  const [pending, startTransition] = useTransition()
  const [fb, setFb] = useState<Feedback>(null)

  const inputCls = "w-full h-8 rounded-lg border border-gray-300 dark:border-white/20 bg-white dark:bg-white/5 px-3 text-[12.5px] dark:text-white placeholder:text-gray-400 outline-none"
  const labelCls = "block text-[11px] font-medium text-gray-600 dark:text-gray-400 mb-1"
  ...
  function handleSubmit() {
    if (!name.trim())    { setFb({ type: "error", msg: "Name is required." }); return }
    if (!lastFour.trim()) { setFb({ type: "error", msg: "Last 4 digits required." }); return }
    startTransition(async () => {
      try {
        await adminCreateAccount(userId, {
          name: name.trim(), lastFour: lastFour.slice(-4), bankName, currency,
          accountType: type === "crypto" ? "crypto" : apy ? "sweep" : "bank",
          routing: routing.trim() || undefined, apy: apy || "0", status,
        })
        onDone()
      } catch (e) {
        setFb({ type: "error", msg: e instanceof Error ? e.message : "Failed." })
  ...
  return (
    <div className="p-3 border border-dashed border-gray-300 dark:border-white/20 rounded-xl space-y-2.5 bg-gray-50 dark:bg-white/3 mt-2">
      ...
      <div>
        <label className={labelCls}>{type === "bank" ? "Bank preset" : "Crypto preset"}</label>
        {type === "bank" ? (
          <select value={bankPresetIdx} onChange={e => applyBankPreset(Number(e.target.value))} className={inputCls}>
            {BANK_PRESETS.map((p, i) => <option key={i} value={i}>{p.bank}</option>)}
          </select>
        ) : (
          <select value={cryptoPresetIdx} onChange={e => applyCryptoPreset(Number(e.target.value))} className={inputCls}>
            {CRYPTO_PRESETS.map((p, i) => <option key={i} value={i}>{p.label}</option>)}
          </select>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <input className={inputCls} placeholder={type === "bank" ? "Operating · USD" : "Bitcoin Wallet"} value={name} onChange={e => setName(e.target.value)} />
        </div>
        <div>
          <label className={labelCls}>Last 4 / ID</label>
          <input className={inputCls} placeholder="4910" maxLength={10} value={lastFour} onChange={e => setLastFour(e.target.value)} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <select value={currency} onChange={e => setCurrency(e.target.value)} className={inputCls}>
            {(type === "bank" ? BANK_CURRENCIES : CRYPTO_CURRENCIES).map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className={labelCls}>{type === "bank" ? "Bank name" : "Network"}</label>
          <input className={inputCls} placeholder={type === "bank" ? "JPMorgan Chase" : "Bitcoin Network"} value={bankName} onChange={e => setBankName(e.target.value)} />
        </div>
        <div>
          <label className={labelCls}>{type === "bank" ? "Routing / IBAN" : "Wallet address"}</label>
          <input className={inputCls} placeholder={type === "bank" ? "021000021" : "1A1zP1…"} value={routing} onChange={e => setRouting(e.target.value)} />
        </div>
      </div>
      ...
      {type === "bank" && (
        <div>
          ...
          <input type="number" min="0" step="0.01" className={inputCls} placeholder="5.21" value={apy} onChange={e => setApy(e.target.value)} />
        </div>
      )}

      <div className="flex gap-2 pt-1">
        <Button size="sm" className="flex-1 h-7 text-[11.5px]" onClick={handleSubmit} disabled={pending}>
          {pending ? <RefreshCwIcon className="h-3 w-3 animate-spin" /> : <PlusIcon className="h-3 w-3" />} Create
        </Button>
        <Button variant="outline" size="sm" className="h-7 text-[11.5px]" onClick={onCancel} disabled={pending}>Cancel</Button>
      </div>
    </div>
...
function AccountRow({ account, onAdjust, onDelete, isAdjusting }: {
  account: Account
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 border-b border-gray-100 dark:border-white/8 last:border-0 text-[12px]">
      <div className={cn("h-7 w-7 rounded-md grid place-items-center text-[12px] shrink-0",
        isCrypto ? "bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400"
                 : "bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-300")}>
        {isCrypto ? (CURRENCY_FLAGS[account.currency] ?? "₿") : <WalletIcon className="h-3.5 w-3.5" />}
      </div>
      <div className="flex-1 min-w-0">
        <div className="font-semibold dark:text-white truncate">{account.name}
          <span className="font-normal text-gray-400 dark:text-gray-500 ml-1.5">•••• {account.lastFour}</span>
        </div>
        <div className="text-[10.5px] text-gray-500 dark:text-gray-400">{account.bankName}</div>
      </div>
      <div className="text-right shrink-0">
        <div className="tabular-nums font-semibold dark:text-white">{fmtBalance(account)}</div>
        {apyVal > 0 && <div className="text-[10.5px] text-emerald-600 dark:text-emerald-400">{apyVal.toFixed(2)}% APY</div>}
      </div>
      <Tag tone={STATUS_TONE[account.status as keyof typeof STATUS_TONE] ?? "neutral"}>
        {account.status.charAt(0).toUpperCase() + account.status.slice(1)}
      </Tag>
      <button
        onClick={() => onAdjust(account)}
        className={cn("h-7 px-2 rounded-md text-[11px] font-medium border transition flex items-center gap-1",
          isAdjusting
...
function StudentAccountRow({ user }: { user: RawUser }) {
  ...
  function handleAdjustDone() { setAdjustTarget(null); refetch() }
  function handleAddDone()    { setShowAddBank(false); setShowAddCrypto(false); refetch() }
  ...
  return (
    <div className="border-b border-gray-200 dark:border-white/10 last:border-0">
      ...
      >
        <div className="h-9 w-9 rounded-full grid place-items-center text-white text-[12px] font-semibold shrink-0"
          style={{ background: "linear-gradient(135deg, hsl(235 70% 58%), hsl(280 60% 40%))" }}>
          {initials}
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-[13px] font-semibold dark:text-white">{user.name}</div>
          <div className="text-[11.5px] text-gray-500 dark:text-gray-400">{user.email}</div>
        </div>
        {open && data && (
          <div className="text-right shrink-0 mr-2">
            ...
            <div className="text-[10.5px] text-gray-400 dark:text-gray-500">{accounts.length} account{accounts.length !== 1 ? "s" : ""}</div>
          </div>
        )}
        {!open && (
        ...
        <div className="bg-gray-50 dark:bg-white/3 border-t border-gray-100 dark:border-white/8 px-5 pb-4 pt-3 space-y-3">
          ...
          {isLoading ? (
            <div className="animate-pulse space-y-2">
              {[0,1,2].map(i => <div key={i} className="h-10 bg-gray-200 dark:bg-white/10 rounded-lg" />)}
            </div>
          ) : (
            <>
              {/* Add account buttons */}
              <div className="flex gap-2">
                <Button size="sm" className="gap-1.5 h-7 text-[11.5px]" onClick={() => { setShowAddBank(v => !v); setShowAddCrypto(false) }}>
                  <PlusIcon className="h-3 w-3" />Bank account
                </Button>
                <Button size="sm" variant="outline" className="gap-1.5 h-7 text-[11.5px]" onClick={() => { setShowAddCrypto(v => !v); setShowAddBank(false) }}>
                  <PlusIcon className="h-3 w-3" />Crypto wallet
                </Button>
                <Button size="sm" variant="outline" className="h-7 text-[11.5px] ml-auto" onClick={() => { setFb(null); refetch() }}>
                  <RefreshCwIcon className="h-3 w-3" />
                </Button>
              </div>

              {showAddBank   && <AddAccountForm userId={user.id} type="bank"   onDone={handleAddDone} onCancel={() => setShowAddBank(false)} />}
              {showAddCrypto && <AddAccountForm userId={user.id} type="crypto" onDone={handleAddDone} onCancel={() => setShowAddCrypto(false)} />}

              {/* Bank accounts */}
              {accounts.filter(a => !CRYPTO_SYMBOLS.has(a.currency)).length > 0 && (
                <div className="rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 overflow-hidden">
                  ...
                  {accounts.filter(a => !CRYPTO_SYMBOLS.has(a.currency)).map(a => (
                    <div key={a.id}>
                      <AccountRow
                        account={a}
                        isAdjusting={adjustTarget?.id === a.id}
                        onAdjust={acc => setAdjustTarget(adjustTarget?.id === acc.id ? null : acc)}
                        onDelete={handleDelete}
                      />
                      {adjustTarget?.id === a.id && (
                        <div className="px-4 pb-3">
                          <AdjustPanel account={a} onDone={handleAdjustDone} onCancel={() => setAdjustTarget(null)} />
                        </div>
                      )}
                    </div>
                  ))}
  ...
  return (
    <>
      <PageHeader
        ...
        title="All Accounts."
        subtitle="Manage bank accounts and crypto wallets for every student. Click a student to expand."
        actions={
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 overflow-hidden">
        ...
        {isLoading ? (
          <div className="p-6 space-y-3 animate-pulse">
            {[0,1,2,3].map(i => <div key={i} className="h-16 bg-gray-100 dark:bg-white/8 rounded-lg" />)}
          </div>
        ) : students.length === 0 ? (
      ...
      <div className="rounded-2xl border border-blue-100 dark:border-blue-900/40 bg-blue-50 dark:bg-blue-950/20 p-4">
        <div className="text-[12px] font-semibold text-blue-800 dark:text-blue-300 mb-1.5">How accounts work</div>
        <ul className="text-[11.5px] text-blue-700 dark:text-blue-400 space-y-1 list-disc list-inside">
          <li>Adjusting a bank account balance updates the student's combined balance on their Accounts page instantly.</li>
          <li>Crypto wallet amounts are displayed with live approximate USD equivalents.</li>
          <li>Use <strong>Deposits / Withdrawals</strong> to create transaction records that also update the main balance overview.</li>
        </ul>
      </div>
    </>
  )
